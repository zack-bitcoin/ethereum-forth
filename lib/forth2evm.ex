defmodule Forth2EVM do
	...
	def words_code(text) do
		IO.puts "words code #{inspect text}"
	...
	def encode_word(word) do 
		...
		word=word |> String.split(" ") |> Enum.filter(&(&1!=""))
		b=Enum.reduce(tl(word), "", &(&2<>" "<>&1))
	" jumpdest "<>hd(word)<>" "<>jumps(remove_spaces(b))<>" 0 mload dup1 32 swap1 sub 0 mstore mload jump " end
